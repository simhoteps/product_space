...
const ForceDirectedGraph = () => {
  ...
  useEffect(() => {
    ...
    const group = Array.from(new Set(data.nodes.map((d) => d.group)));
    const simulation = d3
    ...
    node.on("mouseover", (event, d) => {
        link
        .attr("stroke-opacity", (l) => (l.source === d.id || l.target === d.id ? 1 : 0.1));
    
      node
        .attr("fill-opacity", (n) =>
          n === d || links.some((l) => l.source === d.id || l.target === d.id)
            ? 1
            : 0.1
        );
      /* node.attr("fill", "gray"); */
     /*  node.attr("opacity", 0.1);
      ...
      link
        .attr("stroke", (linkData) =>
          relatedLinks.includes(linkData) ? "#FF0000" : "#999"
        )
        .attr("stroke-width", (linkData) =>
          relatedLinks.includes(linkData) ? 8 : 2
        ); */
    ...
    >
      {tooltip && (
        <Tooltip
          x={tooltip.x}
          y={tooltip.y}
          content1={tooltip.content1}
          content2={tooltip.content2}
        />
      )}
      <svg ref={chartRef} />
    </Stack>
